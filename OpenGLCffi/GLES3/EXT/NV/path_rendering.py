...
@params(api='gles3', prms=['range'])
def glGenPathsNV():
	pass


@params(api='gles3', prms=['path', 'range'])
def glDeletePathsNV(path, range):
	pass


@params(api='gles3', prms=['path'])
def glIsPathNV(path):
	pass


@params(api='gles3', prms=['path', 'numCommands', 'commands', 'numCoords', 'coordType', 'coords'])
def glPathCommandsNV(path, numCommands, commands, numCoords, coordType, coords):
	pass


@params(api='gles3', prms=['path', 'numCoords', 'coordType', 'coords'])
def glPathCoordsNV(path, numCoords, coordType, coords):
	pass


@params(api='gles3', prms=['path', 'commandStart', 'commandsToDelete', 'numCommands', 'commands', 'numCoords', 'coordType', 'coords'])
def glPathSubCommandsNV(path, commandStart, commandsToDelete, numCommands, commands, numCoords, coordType, coords):
	pass


@params(api='gles3', prms=['path', 'coordStart', 'numCoords', 'coordType', 'coords'])
def glPathSubCoordsNV(path, coordStart, numCoords, coordType, coords):
	pass


@params(api='gles3', prms=['path', 'format', 'length', 'pathString'])
def glPathStringNV(path, format, length, pathString):
	pass


@params(api='gles3', prms=['firstPathName', 'fontTarget', 'fontName', 'fontStyle', 'numGlyphs', 'type', 'charcodes', 'handleMissingGlyphs', 'pathParameterTemplate', 'emScale'])
...
@params(api='gles3', prms=['firstPathName', 'fontTarget', 'fontName', 'fontStyle', 'firstGlyph', 'numGlyphs', 'handleMissingGlyphs', 'pathParameterTemplate', 'emScale'])
...
@params(api='gles3', prms=['resultPath', 'numPaths', 'paths', 'weights'])
def glWeightPathsNV(resultPath, numPaths, paths, weights):
	pass


@params(api='gles3', prms=['resultPath', 'srcPath'])
def glCopyPathNV(resultPath, srcPath):
	pass


@params(api='gles3', prms=['resultPath', 'pathA', 'pathB', 'weight'])
def glInterpolatePathsNV(resultPath, pathA, pathB, weight):
	pass


@params(api='gles3', prms=['resultPath', 'srcPath', 'transformType', 'transformValues'])
def glTransformPathNV(resultPath, srcPath, transformType, transformValues):
	pass


@params(api='gles3', prms=['path', 'pname', 'value'])
...
@params(api='gles3', prms=['path', 'dashCount', 'dashArray'])
...
@params(api='gles3', prms=['path', 'fillMode', 'mask'])
def glStencilFillPathNV(path, fillMode, mask):
	pass


@params(api='gles3', prms=['path', 'reference', 'mask'])
def glStencilStrokePathNV(path, reference, mask):
	pass


@params(api='gles3', prms=['numPaths', 'pathNameType', 'paths', 'pathBase', 'fillMode', 'mask', 'transformType', 'transformValues'])
def glStencilFillPathInstancedNV(numPaths, pathNameType, paths, pathBase, fillMode, mask, transformType, transformValues):
	pass


@params(api='gles3', prms=['numPaths', 'pathNameType', 'paths', 'pathBase', 'reference', 'mask', 'transformType', 'transformValues'])
...
@params(api='gles3', prms=['path', 'coverMode'])
...
@params(api='gles3', prms=['numPaths', 'pathNameType', 'paths', 'pathBase', 'coverMode', 'transformType', 'transformValues'])
...
@params(api='gles3', prms=['path', 'commands'])
def glGetPathCommandsNV(path, commands):
	pass


@params(api='gles3', prms=['path', 'coords'])
def glGetPathCoordsNV(path, coords):
	pass


@params(api='gles3', prms=['path', 'dashArray'])
def glGetPathDashArrayNV(path, dashArray):
	pass


@params(api='gles3', prms=['metricQueryMask', 'numPaths', 'pathNameType', 'paths', 'pathBase', 'stride', 'metrics'])
def glGetPathMetricsNV(metricQueryMask, numPaths, pathNameType, paths, pathBase, stride, metrics):
	pass


@params(api='gles3', prms=['metricQueryMask', 'firstPathName', 'numPaths', 'stride', 'metrics'])
def glGetPathMetricRangeNV(metricQueryMask, firstPathName, numPaths, stride, metrics):
	pass


@params(api='gles3', prms=['pathListMode', 'numPaths', 'pathNameType', 'paths', 'pathBase', 'advanceScale', 'kerningScale', 'transformType', 'returnedSpacing'])
...
@params(api='gles3', prms=['path', 'mask', 'x', 'y'])
def glIsPointInFillPathNV(path, mask, x, y):
	pass


@params(api='gles3', prms=['path', 'x', 'y'])
def glIsPointInStrokePathNV(path, x, y):
	pass


@params(api='gles3', prms=['path', 'startSegment', 'numSegments'])
def glGetPathLengthNV(path, startSegment, numSegments):
	pass


@params(api='gles3', prms=['path', 'startSegment', 'numSegments', 'distance', 'x', 'y', 'tangentX', 'tangentY'])
def glPointAlongPathNV(path, startSegment, numSegments, distance, x, y, tangentX, tangentY):
	pass


@params(api='gles3', prms=['matrixMode', 'm'])
...
@params(api='gles3', prms=['path', 'fillMode', 'mask', 'coverMode'])
def glStencilThenCoverFillPathNV(path, fillMode, mask, coverMode):
	pass


@params(api='gles3', prms=['path', 'reference', 'mask', 'coverMode'])
def glStencilThenCoverStrokePathNV(path, reference, mask, coverMode):
	pass


@params(api='gles3', prms=['numPaths', 'pathNameType', 'paths', 'pathBase', 'fillMode', 'mask', 'coverMode', 'transformType', 'transformValues'])
...
@params(api='gles3', prms=['numPaths', 'pathNameType', 'paths', 'pathBase', 'reference', 'mask', 'coverMode', 'transformType', 'transformValues'])
...
@params(api='gles3', prms=['fontTarget', 'fontName', 'fontStyle', 'pathParameterTemplate', 'emScale', 'baseAndCount[2]'])
def glPathGlyphIndexRangeNV(fontTarget, fontName, fontStyle, pathParameterTemplate, emScale, baseAndCount[2]):
	pass


@params(api='gles3', prms=['firstPathName', 'fontTarget', 'fontName', 'fontStyle', 'firstGlyphIndex', 'numGlyphs', 'pathParameterTemplate', 'emScale'])
...
@params(api='gles3', prms=['firstPathName', 'fontTarget', 'fontSize', 'fontData', 'faceIndex', 'firstGlyphIndex', 'numGlyphs', 'pathParameterTemplate', 'emScale'])
...
@params(api='gles3', prms=['program', 'location', 'genMode', 'components', 'coeffs'])
...
@params(api='gles3', prms=['color', 'genMode', 'colorFormat', 'coeffs'])
def glPathColorGenNV(color, genMode, colorFormat, coeffs):
	pass


@params(api='gles3', prms=['texCoordSet', 'genMode', 'components', 'coeffs'])
def glPathTexGenNV(texCoordSet, genMode, components, coeffs):
	pass


@params(api='gles3', prms=['genMode'])
def glPathFogGenNV(genMode):
	pass


@params(api='gles3', prms=['color', 'pname', 'value'])
...
@params(api='gles3', prms=['texCoordSet', 'pname', 'value'])
